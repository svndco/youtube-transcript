...
async function fetchAndCopyTranscript(clipboardy, axios, currentUrl, videoId) {
    ...
    logToFile('Transcript and URL copied to clipboard successfully.');
    return textToCopy;  // Return the clipboard text for logging
}
...
(async () => {
    ...
    const text = await fetchAndCopyTranscript(clipboardy, axios, currentUrl, videoId);
})();
